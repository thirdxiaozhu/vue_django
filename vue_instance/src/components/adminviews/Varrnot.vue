                ...
                <el-table :data="form" tooltip-effect="dark" highlight-current-row style="width: 100%" row-key="id"
                    ...
                    <el-table-column>
                        <template slot-scope="scope">
                            <el-button type="success" @click="operating_id = scope.row.id; dialogVisible=true" :loading="load">安排时间
                            </el-button>
                        </template>
                    </el-table-column>
                </el-table>
...
<script>
    import { initCourseList4test, getRelations,updateChoice,getCollegelist4test, getCoursesAsCollege } from '@/api/axioses'
    export default {
        data() {
            return {
                form: [],
        ...
        methods: {
            initCourse(mod) {
                ...
                initCourseList4test(params).then(res => {
            ...
            collegeChange(){
                ...
                var that = this;
                getCoursesAsCollege(params).then(res =>{
                    if(res.data.code === 1000){
